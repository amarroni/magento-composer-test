...
$modelTaxClass = Mage::getModel('tax/class')->load('Tax France', 'class_name');
if (!$modelTaxClass->getId()) {
    $modelTaxClass = Mage::getModel('tax/class');
    $modelTaxClass->setClassName('Tax France');
    $modelTaxClass->setClassType(Mage_Tax_Model_Class::TAX_CLASS_TYPE_PRODUCT);
    $modelTaxClass->save();
}

$rateModel = Mage::getModel('tax/calculation_rate')->load('FR-*-*-Rate 1','code');
if (!$rateModel->getId()) {
    /* @var $rateModel Mage_Tax_Model_Calculation_Rate */
    $rateModel = Mage::getModel('tax/calculation_rate');
    $rateModel->setCode('FR-*-*-Rate 1');
    $rateModel->setTaxCountryId('FR');
...
$ruleModel = Mage::getModel('tax/calculation_rule')->load('Retail Customer-Taxable Goods-Rate France 1','code');
if (!$ruleModel->getId()) {
    ...
    $ruleModel->setCode('Retail Customer-Taxable Goods-Rate France 1');
